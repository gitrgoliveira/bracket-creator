...
//
// It is the viewer's ONE deep link. A printed tag's QR is this link with one
// entry, `?w=<number>` (helper.playerTagURL, whose Go encoder shares a fixture
// with this module's suite: internal/helper/testdata/tag_watch_links.json).
// The one-person `?player=` / `?name=` reader it generalised was removed on
// 2026-09-23 as duplicated behaviour. Two of that reader's decisions were
...
//
// Id first, then number. The number
...
// contract: the viewer resolves token by token through resolveFreshTokens,
// which also keeps the applied ledger, so a rule belongs here and nowhere
// else. resolveWatchlistTokens below is the batch form of the same call with
// NO production caller; watchlist_link.test.jsx exercises the rule through it,
// which is the only reason it is kept. Do not add a rule there.
