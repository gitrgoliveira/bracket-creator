...
- **Daihyosen** (a representative bout) to settle a tie: in the knockout when a team encounter is level, or in a team pool or league when teams finish equal on every ranking criterion (two or more, with a multi-way tie playing a round-robin of daihyosen bouts). A team pool or league daihyosen is scored as its own single-point bout: it appears on the shiaijo with a **DH** tag, and the score editor lets you pick the player each team fields from its roster. In the pool and league standings, the team that won its daihyosen carries a **DH** badge (shown for the podium places, 1st through 3rd, in a league); tap the badge for a definition.
