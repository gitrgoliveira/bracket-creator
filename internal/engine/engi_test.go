...
func TestRecordEngiMatchResult_PoolFlagMajority(t *testing.T) {
	...
	res, err := eng.recordEngiMatchResult(eng.store, compID, first.ID, 3, 2, "")
	require.NoError(t, err)
...
func TestRecordEngiMatchResult_InvalidRejected(t *testing.T) {
	...
	for _, bad := range [][2]int{{0, 0}, {2, 0}, {1, 1}, {4, 2}, {7, 0}} {
		_, err := eng.recordEngiMatchResult(eng.store, compID, first.ID, bad[0], bad[1], "")
...
func TestComputeEngiStandings_PoolWinsThenFlags(t *testing.T) {
	...
	for _, m := range matches {
		...
		_, err := eng.recordEngiMatchResult(eng.store, compID, m.ID, fa, fb, "")
		require.NoError(t, err)
	}
...
func TestRecordEngiMatchResult_BronzeMatchReachable(t *testing.T) {
	...
	_, err = eng.recordEngiMatchResult(eng.store, compID, sf[0].ID, 3, 2, "")
	require.NoError(t, err, "SF0 engi score must succeed")
	_, err = eng.recordEngiMatchResult(eng.store, compID, sf[1].ID, 3, 2, "")
	...
	bronzeRes, err := eng.recordEngiMatchResult(eng.store, compID, "m-bronze", 5, 0, "")
