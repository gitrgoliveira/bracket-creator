...
// 2026-08-18. draw_ekc_test.go covers the three Junior events; together the two
// files are all seven events on the sheet.
//
...
//
//   - Men Team: blocks of 6 hit the R6(c) split defect. Its shiaijo assignment
//     IS correct and is asserted; its bye allocation is asserted in a test that
//     skips until R6(c) is fixed.
...
// splits that 3+3, so each half owes one bye and R6 hands it to that half's top
// occupant -- on shiaijo A, the seeded pool's winner P1 and then P2. We split
// 4+2 instead, so the 2-side skips round 1 wholesale and the byes land on P3#1
// and P9#2 while the seeded pool plays.
//
