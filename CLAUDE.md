...
- **`web-mobile/`** — Preact/JSX frontend for the mobile app, served embedded in the binary. Entry point: `web-mobile/index.html`. JS modules in `web-mobile/js/` (`admin.js`, `viewer.js`, `app.js`, `api.js`, `data.js`, `bracket.js`). CSS in `web-mobile/css/styles.css`. Pre-compiled to `web-mobile/dist/` by esbuild (run automatically as part of `make go/build`). Key component: `LinedTextarea` in `admin.js` — shows numbered line gutter alongside the participant paste box. `buildRoster(competitions)` in `viewer.jsx` deduplicates players across all competitions and gates `checkedIn` on `checkInEnabled` — badge rendering sites must not check `pRecord.checkedIn` directly without also checking `c.checkInEnabled`; use `buildRoster` instead.
...
- Check-in badge rendering in `viewer.jsx` must go through `buildRoster()` — do not render `pRecord.checkedIn` directly. `buildRoster` masks `checkedIn` to `false` when `checkInEnabled` is `false`, so rendering sites are guaranteed a clean boolean without re-checking the competition flag.
