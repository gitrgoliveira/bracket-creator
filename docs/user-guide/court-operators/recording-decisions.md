...
Recording a kiken or fusenpai changes only that match. It does not do anything to the withdrawn competitor's other scheduled matches: each one shows its own notice and one-tap **Record default win for `<opponent>`** button, described above, when it comes up in the queue or you open it in the score editor. As with **Finish + Start Next**, the court console and the Scores tab then start the court's next match in the same competition, passing over the withdrawn competitor's own. The app never switches you to another competition or court by itself.

You can also open one of the competitor's other matches in the score editor and record **Fusenpai** for the barred competitor. The match records their default loss, and their eligibility record still names the match where they withdrew, so an injury kiken can still be reinstated. That match then offers **Clear default win**, which works as it does for a fusensho: while the competitor is still withdrawn, the match goes back to the queue. Recording a second **Kiken** for a competitor who already withdrew elsewhere is refused: a competitor withdraws only once.
...
In a knockout, a next match that is still in progress must be finished or sent back to the queue before a withdrawal can be cleared. If another match is running on this match's court, the app names it and offers to send it back to the queue, which keeps any score entered for it.
