...
// TestTeamResultFrom_SameNameOpposingFightersAttributedByMemberID pins the
// individual-victory attribution against the one shape a name comparison
// cannot decide: two fighters from OPPOSING teams sharing a display name.
// That is legal (only two members of ONE team may not share one), and the
// name arms match side A first whichever of them won, so before member ids
// existed a bout side B won credited side A an individual victory and could
// hand the encounter to the wrong team.
func TestTeamResultFrom_SameNameOpposingFightersAttributedByMemberID(t *testing.T) {
	subs := []SubMatchResult{{
		Position:       1,
		SideA:          "Sato",
		SideB:          "Sato",
		SideAMemberID:  "member-tora-1",
		SideBMemberID:  "member-kaze-1",
		Winner:         "Sato",
		WinnerMemberID: "member-kaze-1",
		IpponsB:        []string{"M", "K"},
	}}

	line := TeamResultFrom(subs, "Tora", "Kaze")
	require.NotNil(t, line)
	assert.Equal(t, 1, line.ShiroIV, "side B's member won, so side B takes the individual victory")
	assert.Equal(t, 0, line.AkaIV, "side A must not be credited for a bout its namesake opponent won")
}

// The name comparison still serves a row the repair has not reached, so the
// id-first rule did not simply delete the fallback it replaced.
func TestTeamResultFrom_LegacyRowWithoutMemberIDsStillAttributesByName(t *testing.T) {
	subs := []SubMatchResult{{
		Position: 1,
		SideA:    "Tanaka",
		SideB:    "Watanabe",
	...
	line := TeamResultFrom(subs, "Tora", "Kaze")
	require.NotNil(t, line)
	assert.Equal(t, 1, line.ShiroIV, "a row with no member ids is still attributed by name")
