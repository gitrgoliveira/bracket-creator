...
This is the command-line counterpart of the live app's **Export PDFs** button. Both share the same rendering engine; use the CLI when you want PDFs without running the live server, or when you want to generate them on a separate, LibreOffice-equipped machine (see [When to use the CLI](#when-to-use-the-cli)).
