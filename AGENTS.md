...
- **Mobile App (`mobile-app` command):** A real-time tournament management server serving a Preact/JSX UI from `web-mobile/`. State is file-backed: `tournament-data/tournament.md` (YAML) and `tournament-data/competitions/<id>/config.md` + `participants.csv`. Backend in `internal/mobileapp/` (Gin handlers) and `internal/state/` (store). Real-time updates via SSE. Admin actions require `X-Tournament-Password` header. Run with `make run-mobile`.
- **Real-Time Updates:** Real-time updates via SSE. Admin actions require `X-Tournament-Password` header. Run with `make run-mobile`.
- **Admin Auth Modes:** Two startup-selected modes (`internal/mobileapp/auth_source.go`). **File mode** (default) stores the password plaintext in `tournament.md`; a public `POST /api/tournament/reset` resets it without the old password. **Locked mode** (`--lock-password` + `TOURNAMENT_PASSWORD_HASH` env var) compares the header against a bcrypt hash and disables the reset API (`POST /api/tournament/reset` returns 404; the SPA `/reset` page still loads but shows an operator-disabled message). The public `GET /api/auth-config` reports the active mode so the SPA can hide reset UI when locked. Generate hashes with `bracket-creator hash-password`.
- **Pool Scoring Rules:**
