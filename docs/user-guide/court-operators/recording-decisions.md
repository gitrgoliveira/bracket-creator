...
Right after you record a kiken, the score editor lists that competitor's remaining matches under **Remaining matches for** their name: every match of theirs that is not finished, including one already in progress. Each has an **Award default win to opponent** button, which records the same fusensho default win as the queue's button. On the court console the list stays in place after the kiken, even once the court moves on, until you close it with its close button, so you can work through it in one go.

You can also open one of those matches in the score editor and record **Fusenpai** for the barred competitor. The match records their default loss, and their eligibility record still names the match where they withdrew, so an injury kiken can still be reinstated. That match then offers **Clear default win**, which works as it does for a fusensho: while the competitor is still withdrawn, the match goes back to the queue. Recording a second **Kiken** for a competitor who already withdrew elsewhere is refused: a competitor withdraws only once.
