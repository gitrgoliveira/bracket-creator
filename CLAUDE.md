...
- `clock_skew` — the write's `modifiedAt` was more than 5s in the server's future, so it is provably wrong. Nothing was stored, the client resyncs its `/api/time` offset, and re-entering IS the remedy. The copy for the two cases is deliberately opposite; do not merge them.
