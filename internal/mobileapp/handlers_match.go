...
func backfillMatchIdentityForHantei(store CompetitionStore, compID, matchID string, req *state.MatchResult) {
	...
	// as soon as AT LEAST ONE side id is known (domain.WinnerIDAcceptable's
	// only exemption is BOTH ids unknown together; the old broader "only
	// when both side ids are already on the wire" tolerance is gone), so it
	// needs the real stored ids to check against -- the SPA sends winnerId
	// alone (api_serializers.jsx), never sideAId/sideBId, so without this the
	// wire payload would otherwise carry no side ids at all and every
	// ordinary completed-match write would fail that check. Narrowed to the
	// two id fields specifically (PR #416 finding 7), not sideA/sideB too:
	// once both ids are already on the wire, validateWinnerIDMatchesSide has
	// everything it needs without a store round-trip, and the engine's own
	// gate inside the transaction remains the authoritative check either
	// way. Only a WRITE THAT NAMES A WINNER pays the extra read at all (a
	// "running"/in-progress update carries no WinnerID), so the hot
	// in-progress-score path stays free of it, matching the original
	// hantei-only rationale below for that path specifically.
...
func RegisterMatchHandlers(r *gin.RouterGroup, eng *engine.Engine, store CompetitionStore, tx CompetitionTransactor, hub *Hub, verifier PasswordVerifier, tl TournamentLoader) {
	r.POST("/competitions/:id/matches/bulk-score", func(c *gin.Context) {
		...
		// bulkScoreEntry wraps state.MatchResult with a transient, per-entry
		// field that binds from the JSON body but is never part of
		// state.MatchResult itself -- mirrors scoreRequestBody's wrapping of
		// ScoreRequest below for the single-match /score endpoint.
		type bulkScoreEntry struct {
			state.MatchResult
			// ForceDownstreamReopen bypasses bc-kcdg's downstream-knockout-
			// correction guard for THIS entry, same field name and contract as
		...
		for i := range results {
			...
			backfillMatchIdentityForHantei(store, id, results[i].ID, &results[i].MatchResult)
			...
			if err := tx.WithTransaction(id, func(stx state.StoreTx) error {
				...
				case errors.As(err, &downstreamPlayedErr):
					// bc-kcdg: correcting THIS entry would repaint a downstream
					// match that already carries its own result. Unlike the
					// single-match endpoints, a batch entry has no dedicated 409
					// to answer with (this endpoint is always 200, partial
					// success), so the discriminator rides in Reason instead: the
					// operator can retry the same entry with
			...
			// bc-kcdg: each reopened match is distinct from the entry just
	...
	r.PUT("/competitions/:id/matches/:mid/quick-score", func(c *gin.Context) {
		...
		if err != nil {
			...
			// winner while a downstream match carries a result of its own.
	...
	// (mp-gmcg, spec 006 decision 4), and since bc-tmfn for any completed
	// match a withdrawal (kiken, fusenpai) decided, team or individual: the
	// way to clear a withdrawal recorded by mistake (engine.ReopenMatch).
	...
	r.POST("/competitions/:id/matches/:mid/requeue-blocker-and-reopen", func(c *gin.Context) {
		...
		if err != nil {
			...
			case errors.Is(err, engine.ErrReopenNotCompleted),
				...
				// RequeueBlockerAndReopen) — i.e. with the blocker already
...
func registerScoreHandler(r *gin.RouterGroup, eng ScoringEngine, store CompetitionStore, tx CompetitionTransactor, hub Broadcaster, verifier PasswordVerifier, tl TournamentLoader) {
	...
	r.PUT("/competitions/:id/matches/:mid/score", func(c *gin.Context) {
		...
		if engErr != nil {
			...
			// bc-kcdg: correcting this match would change an already-propagated
			// winner while a downstream match carries a result of its own. Fixed
			// wire contract, shared with every other knockout-correction write
			// (see respondIfDownstreamKnockoutPlayed's doc comment).
			if respondIfDownstreamKnockoutPlayed(c, engErr) {
				return
			}
