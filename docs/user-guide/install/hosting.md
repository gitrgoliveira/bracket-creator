...
| [GCP free tier](https://github.com/gitrgoliveira/bracket-creator/tree/main/deploy/gcp) | GCP Always-Free e2-micro (1 GB RAM, x86-64), Terraform-deployed | Club-sized events; 1 GB/month egress cap | You want a free cloud VM. Watch the egress cap: a busy day with a few hundred connected viewers can exceed it. |
| [Oracle free tier](https://github.com/gitrgoliveira/bracket-creator/tree/main/deploy/oracle) | Oracle Always-Free Ampere A1 (2 OCPU / 12 GB, Arm64), Terraform-deployed | 1000+ connected viewers; 10 TB/month egress | You need the free-forever tier that matches the app's large-event SSE target. |
...
Any deployment reachable over the internet should run in locked mode rather than the default file mode. Locked mode reads a bcrypt hash from an environment variable and disables the public password-reset endpoint. Refer to [Admin authentication](../organisers/operating-modes.md#admin-authentication-mode) for the setup steps.
