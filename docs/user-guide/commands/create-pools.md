...
| `--number-prefix` | `-n` | (none) | Letter prefix for competitor numbers (for example, `K` produces K1, K2, …). When omitted, one is derived from `--title-prefix`; if `--title-prefix` is also omitted, the prefix is `K`. The CLI derives this from `--title-prefix` alone, with no knowledge of any other competition, so running this command more than once for the same tournament without a distinct `-n` each time produces the same K1, K2, … tags for every one of them; pass a distinct `-n` per competition (the tournament mobile app, unlike this command, checks prefixes already in use and avoids them automatically). Numbers follow the order of the input file, the first row is K1, and the draw never renumbers anyone, so the numbers within a pool are not consecutive |
