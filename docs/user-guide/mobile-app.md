...
- **Excel (`.xlsx`)**: two workbooks, both always available with no extra dependencies. **Download results** produces a workbook with the played scores, pool standings, winners, and match decisions (Kiken / Fus. / DH / Ht) filled in, for a shareable archive of a finished or in-progress competition. It covers pools, league, and knockout (playoffs) competitions; Swiss competitions have no static bracket, so follow their live standings instead. **Download blank template** produces the empty bracket workbook (pool draws, matches, and trees) with linked formulas, for scoring by hand.
