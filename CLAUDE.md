...
- **`cmd/`**: Cobra CLI commands. Each uses an options struct with a `run()` method. `create-pools` and `create-playoffs` share significant logic. Shared helpers (`processEntries`, `openOutputFile`, `resolveNumberPrefix`) live in `cmd/shared.go`; competitor numbering itself is `helper.AssignPlayerNumbers`.
