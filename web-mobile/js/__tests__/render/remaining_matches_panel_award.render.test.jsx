...
// recordDecision arguments: fusensho (not fusenpai, which the server refuses
// with 409 already_ineligible for a competitor already withdrawn elsewhere),
...
async function recordTanakaKiken() {
  await mount();
...
describe('RemainingMatchesPanel.award (bc-rawm)', () => {
  it('lists the remaining scheduled match and awards fusensho with the withdrawn side and a reason naming them', async () => {
    ...
    // fusensho, not fusenpai: fusenpai would be refused (409
    // already_ineligible) for a competitor already withdrawn elsewhere.
    ...
    expect(screen.getByText('No remaining scheduled matches.')).toBeTruthy();
  });

  it('shows no remaining matches when Tanaka has none scheduled', async () => {
    window.compMatchesForCompetition = vi.fn().mockReturnValue([]);
    await recordTanakaKiken();
    await waitFor(() => expect(screen.getByText('No remaining scheduled matches.')).toBeTruthy());
  });
