...
func (e *Engine) runDrawPipeline(id string) error {
	...
	// BuildPoolPhaseTreeAware*), so a standalone knockout or Swiss
	// competition over a legacy blank-dojo roster drew silently: neither
	// generateKnockout (helper.StandardSeeding has no dojo opinion at all)
	...
	_, err = e.store.UpdateCompetitionChanged(id, func(current *state.Competition) (*state.Competition, error) {
		...
		//   - NumberPrefix (player numbering in generatePools; generateKnockout
