...
This rule applies to every format that carries numbers: mixed, league and knockout-only. A Swiss competition's competitors carry no number at all.

A competitor's number is created when you apply the participants list: it is the competition's prefix plus the row's position in that list, so with the prefix K the first row is K1, the second K2, and so on. The check-in list shows it from that moment, and it is the number: generating or discarding the draw does not change it. The draw copies each competitor's number into their pool, so the numbers within one pool are not consecutive; the pool sheet, the printed tags, the Names to Print cards, the viewer and the export all show the same number the desk called.

The number follows the list, so editing the list before the draw can change it: adding a competitor appends a number at the end, while removing a row, or clicking **Shuffle unseeded**, shifts the numbers of every row after it. Once the draw is generated the list is locked, and the numbers with it. If you print tags before the draw, print them after the list is final.
...
You can change the number prefix at any time, including after the draw is generated or the competition has started. Saving the new prefix changes the letters of every competitor's number and nothing else: positions, pools and pairings stay as they are. If you have already printed tags, reprint them after changing the prefix, because the numbers on the old tags no longer match.
