...
// handlers_squad.go (bc-tmid pass 1): a team's squad, the people on it,
// each with a stable id and a display index (domain.TeamMember), kept in
// its own per-competition file rather than in Player.Metadata. Mirrors the
// LoadSquads / AddTeamMember / RenameTeamMember methods on *state.Store.
