...
- For a kachinuki team encounter, the editor shows the recorded bouts with a **Reopen match** button. Reopen returns the encounter to running with its bout log intact, so you can carry on or fix a bout and then end it again. Because reopening discards the recorded result, it asks for a reason first, in the same way as a correction, and the reason is kept with the result.

![Reopen prompt asking for a reason, with a preset list and an optional note](../../screenshots/kachinuki-reopen-reason.png)

Reopening is refused while another match is already running on the same court: putting the reopened encounter back into play would leave the court with two live matches. Finish the running match, or send it back to the queue, and then reopen.
