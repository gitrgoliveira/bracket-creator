...
- **Seeding** (`helper/seed.go`): `StandardSeeding()` uses `generateBracketOrder()` for placement. `ApplySeeds()` handles collisions by swapping seed values. Names are matched after `NormalizeParticipantName` and title-casing (helper/dedup.go, helper/tournament.go), so case and accents do not matter. `delayDojoMeetings` (same file) then delays dojo-mate meetings across the unseeded slots; its acceptance is SUM-DRIVEN by operator ruling (2026-08-31): a swap may pull an already-deep dojo's first meeting one round earlier to push the draw's earliest meetings later. Do NOT add a per-dojo never-earlier veto here: that rule belongs to the POOL exchange pass (`improveDojoMeetings`), and a finding that reports the deep-dojo trade as a regression is re-litigating the ruling.
