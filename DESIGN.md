...
2. **Kendo first.** Red (Aka) and White (Shiro) are positional, never swapped. Web bracket cards put Aka first/top; the horizontal scoreboard and list rows put Shiro left / Aka right; Excel puts White in the left column. The two sides must be **distinguishable at a glance on every surface**: the distinction is carried by *treatment*, not hue alone (so it survives glare, projectors, and color-blindness): Aka = solid/red-tinted fill, Shiro = framed white (cool border + diagonal hatch) so "white" never dissolves into the page. **Color area must scale with the surface.** A full-screen scoreboard can flood half the screen; a dense schedule row cannot rely on a 5px spine. Give each side a tinted cell or a colored header so the signal stays legible as the component shrinks. **Not a badge**: a tinted surface carries no SHIRO/AKA badge (operator ruling 2026-09-20, bc-sccl), and every one in the app was removed with it. See [§4 Match cards](#match-cards--bc-match), [§4 Aka/Shiro side treatment](#akashiro-side-treatment), and [CLAUDE.md](CLAUDE.md) "Match Colors" for the full rule.
...
| 34px | The watchlist hero's court letter (`.wl-hero__where-v`), and nothing else. The single token telling a competitor where to walk, on a card read at arm's length in a noisy hall; it tops the scale deliberately (bc-wlhc, operator decision 2026-09-20) |
...
| Side | Fill | Edge / frame | Naming | Position |
|---|---|---|---|---|
| **Shiro** (White) | `--white-side` (`#f6f7fb`) or framed pure-white | `--accent` navy border + 45° `--shiro-hatch` diagonal (use `--shiro-edge` only on dark TV/overlay surfaces) | no badge: the tint plus the hatch carry it. Name the side in text with an `sr-only` span (never an `aria-label` on a bare `<div>`, where ARIA prohibits naming). One exception, by SIZE: the watchlist hero (`.wl-hero__side-lbl`) is a large personal card, not a dense row, and keeps a visible word | **left** (horizontal) / **bottom** (bracket tree) |
| **Aka** (Red) | `--red-soft` tint, or solid `--red` when emphasized | `--red` | no badge: the tint carries it. Same `sr-only` rule | **right** (horizontal) / **top** (bracket tree) |

The hatch on Shiro is load-bearing, and more so now that the badge is gone: pure white on a white card is invisible, `--white-side` alone is too faint on dense rows, and the hatch is the one NON-COLOUR cue that lets the text badge go. The diagonal hatch (`repeating-linear-gradient(-45deg, …)`) plus the navy frame/leading-bar is what makes "white" register as a deliberate side. (Until 2026-09-20 this section specified a framed-white `#fff` + `--accent` badge; bc-sccl removed every one, so that rule now has no subject.)
...
6. **Match the prefix.** Pick the existing prefix that covers your concept (`bc-`, `pool-`, `sched-`, `vsched-`, `tcard-`, `viewer-`, `score-`, `running-`, `my-match-`, `wl-`) before inventing a new one.
