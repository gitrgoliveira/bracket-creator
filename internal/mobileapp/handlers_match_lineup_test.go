...
// TestMatchLineupPUT_DuplicateMemberRejected pins the duplicate-member guard
// (bc-dnst) at the match-scoped PUT: the same squad member id at two
// positions is a domain validation error and must reach the client as 400,
// exactly like an invalid position key.
func TestMatchLineupPUT_DuplicateMemberRejected(t *testing.T) {
	r, store, _ := setupLineupTestRouter(t)
	require.NoError(t, store.SaveTournament(&state.Tournament{Name: "T", Password: "secret"}))
	require.NoError(t, store.SaveCompetition(&state.Competition{ID: "c1", TeamSize: 5}))
	...
	body, _ := json.Marshal(LineupRequest{
		...
		MemberIDs: map[domain.Position]string{
			domain.PosSenpo: memberID,
			domain.PosJiho:  memberID,
		},
	})
	req := httptest.NewRequest(http.MethodPut,
		"/api/competitions/c1/teams/teamA/match-lineups/PoolA-0", bytes.NewReader(body))
	...
	assert.False(t, found, "a rejected write must not persist")
}
