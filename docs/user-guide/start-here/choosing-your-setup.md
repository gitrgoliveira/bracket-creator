...
- **Display screens are optional.** Without them you are running at the partially connected level described above, and results still reach phones.
- **The court display is usually driven from the operator's own machine** over an HDMI cable rather than from a separate device, which keeps the board alive if the venue Wi-Fi drops. See [Host the tournament app](../install/hosting.md) for the deployment choices behind this.
