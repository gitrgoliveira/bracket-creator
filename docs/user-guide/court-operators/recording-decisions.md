...
Recording a kiken or fusenpai changes only that match. It does not do anything to the withdrawn competitor's other scheduled matches: each one shows its own notice and one-tap **Record default win for `<opponent>`** button, described above, when it comes up in the queue or you open it in the score editor.
