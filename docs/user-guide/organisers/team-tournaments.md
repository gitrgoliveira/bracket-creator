...
| A tie involving one Taisho | The Taisho retires like anyone else. If their team now has nobody left, use **Add next bout manually**: pick the surviving team's next fighter, leave the other side empty, give them the walkover (**Fusensho**), then **End match** on that point. (The app never adds a one-sided bout on its own: only you know whether the team is really out of fighters.) | The Taisho stays on. Use **Add next bout manually** and pair the same Taisho against the opponent's next fighter. |
