...
//
// Finding 3 fix: bracketMatchCount returns the count of court-time-consuming
// bracket matches (those NOT auto-marked Completed at generation time), not
// NextPow2(players)-1 (total slot count). For power-of-2 fields this equals
// players-1, but for non-power-of-2 fields with paired byes (e.g. N=6→6,
// N=12→12) it differs from the naive players-1 formula.
//
