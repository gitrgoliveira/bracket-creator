...
func TestMatchLabel_NamesTheBronzeRatherThanItsID(t *testing.T) {
	...
	assert.Equal(t, "knockout Match 4", MatchLabel(ReopenedMatch{ID: state.BronzeMatchID, Number: 4}),
		"a number, when one exists, still wins")
	assert.Equal(t, "knockout Match 7", MatchLabel(ReopenedMatch{ID: "m-r3-0", Number: 7}))
