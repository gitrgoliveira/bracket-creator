...
- **Encho** keeps the same pair fighting on that bout until one of them takes a point. Only the last bout, the two taisho against each other, may go to encho, so the editor offers it only for that pairing. A tie between any other pair is settled with **Record bout** instead. Use Encho whenever your rules say the taisho pairing must have a result, in any stage. The app knows who each team's taisho is from its lineup: the last fighter the lineup places. If you picked a later fighter out of the lineup's order, the fighter left with nobody after them still to fight counts as the taisho. When it cannot tell, it offers Encho rather than guess. That happens when a team has no lineup saved, or the lineup does not place one of the two fighters, for example a lineup with only its first fighter entered. Tapped Encho by mistake, or twice? **Undo encho**, beside it, takes back one overtime period per tap while nothing has been scored in overtime; taking back the last one returns the bout to its tie.
