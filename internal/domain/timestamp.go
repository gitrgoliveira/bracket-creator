...
// newer-or-equal incoming applies and a strictly older one is dropped (an equal
// timestamp applies so a reconnect replay of the same change is idempotent
...
// It lives in domain (the dependency leaf) so both the engine write path and the
// HTTP handlers can share one implementation.
