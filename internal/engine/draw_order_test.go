...
// Court field, never the leaf order, but the ruling asks it be checked), and
// with/without seeds.
	...
	for _, n := range entrantCounts {
		for _, courts := range courtCounts {
			for _, seeded := range []bool{false, true} {
				tName := fmt.Sprintf("%d entrants %d courts seeded=%v", n, courts, seeded)
				t.Run(tName, func(t *testing.T) {
					...
					compID := fmt.Sprintf("draw-order-%d-%d-%v", n, courts, seeded)
