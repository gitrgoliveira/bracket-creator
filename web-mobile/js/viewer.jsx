      ...
      <table className="pool-matrix">
        <thead>
          <tr>
            ...
            {players.map((p, i) => (
              <th key={p.name} className={`pool-matrix__col-head${isHighlighted(p) ? " pool-matrix__col--me" : ""}`} title={p.name}>{p.number || (i + 1)}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {players.map((rowPlayer, ri) => (
            <tr key={rowPlayer.name} className={isHighlighted(rowPlayer) ? "pool-matrix__row--me" : ""}>
              <td className="pool-matrix__row-head" title={rowPlayer.name}>
                <span className="pool-matrix__num">{rowPlayer.number || (ri + 1)}</span>
