...
- **Chusen** (drawing lots) is the last resort when three or more tied teams play a round of daihyosen that still ends level (a cycle, where each beats another). The Pools tab then shows a **"Chusen (drawing lots) required"** panel listing the tied teams: draw lots and enter each team's finishing position, then record it to settle the order and let the competition advance. This is the only place ranks are set by hand, and only when the bouts cannot decide it.
