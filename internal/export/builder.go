...
func BuildResultsWorkbook(store *state.Store, eng *engine.Engine, compID string) ([]byte, error) {
	...
	//    phase: the IsKnockoutEnabled gate below drops the phantom bracket a
	...
	//    pure knockout competition -- and is shared with the blank-template export
	...
	if draw != nil && comp.IsKnockoutEnabled() {
		...
		if bracket != nil {
			...
			// Knockout have no pool data sheet, so the pool-oriented renderer emits
			...
			if len(pools) == 0 && comp.Format == state.CompFormatKnockout {
