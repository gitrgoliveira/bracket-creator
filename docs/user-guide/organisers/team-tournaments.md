...
| A tie involving one Taisho | The Taisho retires like anyone else. If their team now has nobody left, tap **Record bout**: the app adds the walkover bout, the surviving team's next fighter with the other side left empty. Give that fighter the walkover (**Fusensho**), then **End match** on that point. | The Taisho stays on. Tap **Record bout**: the app adds the next bout with the opponent's next fighter and one side empty; put the same Taisho back in the empty side (or use **Add next bout manually**). |
