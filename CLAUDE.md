...
- **A writer must never create the competition directory** (`internal/state/overrides.go`): `saveOverridesLocked` used to `os.MkdirAll` before writing, so an override save landing after `DeleteCompetition` rebuilt `competitions/<id>/` around a lone `overrides.json`. That orphan survives the delete (`ListCompetitions` returns every directory under `competitions/`) and, because IDs are name slugs, a same-named recreation adopts the dead competition's overrides. **Locking cannot fix this**: the save does not have to interleave with the delete to resurrect the directory, only to run after it. Every sibling saver already relies on the directory existing, and `atomicWriteFile` opens its temp with `O_CREATE` but never creates the parent, so a write to a deleted competition correctly fails with ENOENT. The interleaving half is closed separately: `overrides.json` is the one competition file whose writers serialize on the store-wide `s.mu` rather than the per-competition lock, so `DeleteCompetition` takes **both**, in the order compLock then `s.mu` (`s.mu` is a sink: nothing holding it acquires a comp lock, so the edge cannot close a cycle). **Do not "tidy" this by moving overrides onto the per-competition lock**: `computeStandingsFrom` runs inside `WithTransaction`, which already holds that lock, and reads overrides via `e.store.LoadOverrides`, so it would self-deadlock the live scoring path on a non-reentrant mutex.
