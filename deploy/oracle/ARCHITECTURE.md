...
runs the tournament app **continuously, at no cost**, and comfortably handles **large events**, including audiences of 1000+ concurrent live viewers. It is the recommended choice when you expect a
...
> Oracle's Always Free Ampere allowance is **2 OCPU / 12 GB** as of mid-June 2026 (reduced from an earlier 4 OCPU / 24 GB). The deployment requests 2 OCPU / 12 GB to stay within the free limit.
...
- **Two firewall layers** must both allow web traffic, and the deployment configures both for you:
  ...
  2. The **on-host firewall** inside Oracle's Linux images, which blocks web ports by default (a common Oracle pitfall). The deployment opens and persists the necessary rules automatically.
...
- **Availability note:** Always Free Ampere capacity is occasionally unavailable in busy regions
  ("out of host capacity"). If that happens, retry or try a different availability domain; the README explains how.
